...
import { API, GameActionRequest, GameName, GameSettings, GameSettingsSetRequest, GameState, GameRequest, RoomData } from '@gandogames/shared/dto';
...
export class GameService {
	...
	public getGameState(gameId: GameName, roomId?: string): Promise<GameState | null> {
		const request: GameRequest = { roomId };
		return this.backend.call(API.game.state, { params: { gameId }, body: request });
	}

	public gameAction(gameId: GameName, action: string, data?: unknown, roomId?: string): Promise<GameState | null> {
	...
	public setGameSettings(gameId: GameName, settings: GameSettings, roomId?: string): Promise<RoomData> {
	...
	public reset(gameId: GameName, roomId?: string): Promise<RoomData> {
		const request: GameRequest = { roomId };
